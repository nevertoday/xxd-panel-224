...
사진의 정신을 중국 자기 파편으로 다시 맞춘다. 깨짐, 빈틈, 유색, 그리고 넓은 여백.
...
개인 사진 정리, 독립 출판, 전시 습작, 문화 브랜드 비주얼에 맞다. 상단은 알아볼 수 있는 원사진을 남기고, 하단은 자기 필터가 아니다. 의미·정서·은유를 읽은 뒤 유면, 태토 단면, 균열, 틈새로 주체를 다시 맞춘다.

시적이고 절제되며 동시대적이고 동양적인, 심리적 긴장과 소장감이 있는 화면: 작은 인장 같은 주체와 넓은 자기 흰 여백. 그대로의 복제, 화면을 채우기, 전통 문양 쌓기, 골동 진열, 값싼 국조 그래픽, 3D 플라스틱은 피한다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-224`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-224 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-224-prompt.en.md)와 [중국어](references/xxd-panel-224-prompt.zh-CN.md)를 확인하세요.
...
- `design-only`: 전체 캔버스에 Panel 224의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
