...
写真の精神を中国磁器の破片で再構成する。欠け、隙間、釉色、そして大きな余白。
...
個人の写真整理、自主出版、展示の習作、文化ブランドのビジュアルに。上半分は識別できる原写真を残し、下半分は磁器フィルターではない。意味・情緒・隠喩を読んだうえで、釉面・胎土の断面・ひび・隙間で主体を組み直す。

詩的で抑制され、現代的で東洋的、心理的緊張と收藏感のある画面：小さな印章のような主体と大きな磁白の余白。逐物の複製、画面の埋め尽くし、伝統文様の積み重ね、骨董展示、安価な国潮、3Dプラスチックは避ける。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-224` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-224 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-224-prompt.en.md)／[中国語](references/xxd-panel-224-prompt.zh-CN.md)を参照してください。
...
- `design-only`：全画面を Panel 224 のデザイン翻訳にし、写真は見えない参照にします。
